# 🚨 HỆ THỐNG PHÁT HIỆN TÉ NGÃ NGƯỜI CAO TUỔI TẠI GIA ĐÌNH (YOLO26-Pose + Zalo Alert)

Hệ thống giám sát an toàn thụ động theo thời gian thực (Real-time Passive Monitoring System), tự động phát hiện sự cố té ngã ở người cao tuổi bằng AI (YOLO26-Pose) và gửi cảnh báo khẩn cấp (kèm 1 Ảnh Snapshot + 1 Clip Video 3–5 giây bằng chứng) trực tiếp về ứng dụng **Zalo cá nhân** của người chăm sóc.

---

## 🌟 ĐẶC ĐIỂM NỔI BẬT

- **AI Nhận Diện Thời Gian Thực**: Sử dụng mô hình Deep Learning **YOLO26-Pose** trích xuất 17 điểm mốc khớp xương (COCO Keypoints) đạt tốc độ $\ge 25$ FPS.
...
│
├── models
│   └── best.pt                 # Trọng số mô hình AI YOLO26-Pose
│
├── src
│   ├── camera.py               # Quản lý luồng Webcam & Bộ đệm xoay vòng 3-5s
│   ├── detector.py             # Trích xuất 17 điểm mốc (YOLO26-Pose)
