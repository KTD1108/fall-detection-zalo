...
class YOLOPoseDetector:
    """
    Phân hệ Ước lượng Tư thế & Nhận diện Mô hình AI (YOLO26-Pose Detector)
    - Tải mô hình YOLO26-Pose (tự động tải trọng số pre-trained nếu chưa có).
    ...
    def __init__(self, model_path="models/yolo26n-pose.pt", conf_threshold=0.45, device="auto", logger=None):
        ...
        if self.logger:
            self.logger.info(f"Đang khởi tạo mô hình YOLO26-Pose từ [{self.model_path}]...")
            
        try:
            self.model = YOLO(self.model_path)
            if self.logger:
                self.logger.info(" Đã tải thành công mô hình YOLO26-Pose AI!")
        except Exception as e:
            if self.logger:
                self.logger.warning(f"Lỗi tải YOLO26 ({e}). Đang tự động tải mô hình mặc định 'yolo26n-pose.pt'...")
